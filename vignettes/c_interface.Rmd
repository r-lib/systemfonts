...
use `get_cached_face()`. It will look in the cache for a face and size that 
matches your request and assign that to the passed in face pointer that if 
found. If not, it will load it for you and add it to the cache, before returning
it to you. `get_cached_face()` returns 0 if successful.
